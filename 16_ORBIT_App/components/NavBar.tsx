...
export default function NavBar() {
  ...
  const { config, focusMode } = useWorkMode();
  const isActive = (href: string) => href === "/" ? pathname === "/" : pathname.startsWith(href);
  const modeNav = config.navigationItems;
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[224px] flex-col border-r border-black/10 bg-[#fffdf8]/92 px-3.5 py-4 backdrop-blur-2xl lg:flex">
      ...
      <div className="mt-4"><WorkModeSelector /></div>
      <div className="mt-5 px-2"><p className="command-label">Navigation</p></div>
      <nav className="mt-3 space-y-1">{modeNav.map((item, i) => { const active = isActive(item.href); return <Link key={`${item.href}-${i}`} href={item.href} className={`group flex items-center gap-2.5 rounded-[15px] px-2.5 py-2.5 text-[13px] font-bold ${active ? "bg-[#e6edcd] text-[#151515] shadow-[inset_0_0_0_1px_rgba(21,21,21,0.07)]" : "text-black/56 hover:bg-black/[0.04] hover:text-black"}`}><span className={`flex h-7 w-7 items-center justify-center rounded-[10px] ${active ? "bg-white/75" : "bg-black/[0.04] group-hover:bg-white/70"}`}><CommandIcon name={item.icon as CommandIconName} className="h-4 w-4" /></span><span>{item.label}</span></Link>; })}</nav>
      {!focusMode && <div className="mt-6 border-t border-black/8 px-2 pt-5"><p className="command-label">Pilotage ORBIT</p><div className="mt-3 space-y-1">{PILOT_ITEMS.map((item) => { const active = isActive(item.href); return <Link key={item.href} href={item.href} className={`flex items-center gap-2 rounded-[13px] px-2 py-2 text-xs font-bold ${active ? "bg-[#f5effd] text-black" : "text-black/55 hover:bg-black/[0.04] hover:text-black"}`}><CommandIcon name={item.icon} className="h-4 w-4" /> {item.label}</Link>; })}</div></div>}
      {!focusMode && <div className="mt-6 border-t border-black/8 px-2 pt-5"><p className="command-label">Raccourcis</p><div className="mt-3 space-y-1"><Link href="/projects/new" className="flex items-center gap-2 rounded-[13px] px-2 py-2 text-xs font-bold text-black/55 hover:bg-black/[0.04] hover:text-black"><CommandIcon name="plus" className="h-4 w-4" /> Nouveau projet</Link><Link href="/projects" className="flex items-center gap-2 rounded-[13px] px-2 py-2 text-xs font-bold text-black/55 hover:bg-black/[0.04] hover:text-black"><CommandIcon name="library" className="h-4 w-4" /> Bibliothèque</Link></div></div>}
    ...
    <nav className="fixed inset-x-2 bottom-[max(0.5rem,env(safe-area-inset-bottom))] z-50 grid grid-cols-5 gap-0.5 rounded-[21px] border border-black/10 bg-[#fffdf8]/95 p-1 shadow-[0_14px_35px_rgba(33,31,26,0.16)] backdrop-blur-2xl lg:hidden">{modeNav.slice(0, 5).map((item, i) => { const active = isActive(item.href); return <Link key={`${item.href}-${i}`} href={item.href} className={`flex min-h-[49px] flex-col items-center justify-center gap-0.5 rounded-[16px] text-[8px] font-black ${active ? "bg-[#e6edcd] text-black" : "text-black/44"}`}><CommandIcon name={item.icon as CommandIconName} className="h-[17px] w-[17px]" /><span>{item.label}</span></Link>; })}</nav>
  </>;
}
